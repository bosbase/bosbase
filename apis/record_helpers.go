...
import (
	...
	"github.com/bosbase/bosbase-enterprise/mails"
...
const (
	...
	superuserStaticTokenDurationEnvKey = "PB_SUPERUSER_STATIC_TOKEN_DURATION"
)

const defaultStaticAuthTokenDuration = 100 * 365 * 24 * time.Hour

var configuredSuperuserStaticTokenDuration = loadSuperuserStaticTokenDuration()
...
func RecordAuthResponse(e *core.RequestEvent, authRecord *core.Record, authMethod string, meta any) error {
	token, tokenErr := newRecordAuthToken(authRecord)
	if tokenErr != nil {
...
func newRecordAuthToken(record *core.Record) (string, error) {
	if record.IsSuperuser() {
		return record.NewStaticAuthToken(configuredSuperuserStaticTokenDuration)
	}

	return record.NewStaticAuthToken(defaultStaticAuthTokenDuration)
}

func loadSuperuserStaticTokenDuration() time.Duration {
	raw := strings.TrimSpace(os.Getenv(superuserStaticTokenDurationEnvKey))
	if raw == "" {
		return defaultStaticAuthTokenDuration
	}
