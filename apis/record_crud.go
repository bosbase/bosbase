...
func bindRecordCrudApi(app core.App, rg *router.RouterGroup[*core.RequestEvent]) {
	...
	subGroup.GET("/count", recordsCount)
	subGroup.POST("/search", recordVectorSearch).Bind(dynamicCollectionBodyLimit(""))
...
func recordsList(e *core.RequestEvent) error {
	...
	// use rowid when available (SQLite only) to minimize the need of a covering index with the "id" field
	// PostgreSQL doesn't have _rowid_, so we use the default "id" column for PostgreSQL
	if !collection.IsView() {
		driver := core.BuilderDriverName(e.App.NonconcurrentDB())
		// Only use _rowid_ for SQLite databases
		if driver == "sqlite" || driver == "sqlite3" {
			searchProvider.CountCol("_rowid_")
		}
		// For PostgreSQL and other databases, the default "id" column will be used
	}
	...
	return e.App.OnRecordsListRequest().Trigger(event, func(e *core.RecordsListRequestEvent) error {
		...
		//
		// In all cases it doesn't really matter that much because it doesn't affect the builtin BosBase security sensitive fields (e.g. password and tokenKey) since they
...
func recordsCount(e *core.RequestEvent) error {
	collection, err := e.App.FindCachedCollectionByNameOrId(e.Request.PathValue("collection"))
	if err != nil || collection == nil {
		return e.NotFoundError("Missing collection context.", err)
	}
	...
	// Use search provider to get count efficiently
	// Set perPage to 1 to minimize data fetched while still calculating total count
	searchProvider.PerPage(1)

	dummyRecords := []*core.Record{}
	result, err := searchProvider.ParseAndExec(e.Request.URL.Query().Encode(), &dummyRecords)
	if err != nil {
		return firstApiError(err, e.BadRequestError("", err))
	}

	// Return the count from the result
	return e.JSON(http.StatusOK, map[string]any{"count": result.TotalItems})
}
...
func applyCreateAuditFields(record *core.Record, info *core.RequestInfo) {
	authId := requestAuthID(info)

	if record.Collection().Fields.GetByName(core.FieldNameCreatedBy) != nil {
		if authId != "" {
			record.Set(core.FieldNameCreatedBy, authId)
		}
	}

	if record.Collection().Fields.GetByName(core.FieldNameUpdatedBy) != nil {
		if authId != "" {
			record.Set(core.FieldNameUpdatedBy, authId)
		}
	}
}

func applyUpdateAuditFields(record *core.Record, info *core.RequestInfo) {
	original := record.Original()

	if record.Collection().Fields.GetByName(core.FieldNameCreatedBy) != nil {
		record.Set(core.FieldNameCreatedBy, original.GetString(core.FieldNameCreatedBy))
	}

	if record.Collection().Fields.GetByName(core.FieldNameUpdatedBy) != nil {
		if authId := requestAuthID(info); authId != "" {
			record.Set(core.FieldNameUpdatedBy, authId)
		} else {
			record.Set(core.FieldNameUpdatedBy, original.GetString(core.FieldNameUpdatedBy))
		}
	}
}

func requestAuthID(info *core.RequestInfo) string {
