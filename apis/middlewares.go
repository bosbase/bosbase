...
func getAuthTokenFromRequest(e *core.RequestEvent) string {
	...
	if qToken := e.Request.URL.Query().Get("token"); qToken != "" {
		return qToken
	}

	if cookie, err := e.Request.Cookie("pb_auth"); err == nil && cookie != nil {
		if decoded, err := url.QueryUnescape(cookie.Value); err == nil {
			var payload struct {
				Token string `json:"token"`
			}
			if err := json.Unmarshal([]byte(decoded), &payload); err == nil && payload.Token != "" {
...
func logRequest(event *core.RequestEvent, err error) {
	...
	if strings.HasPrefix(event.Request.URL.Path, "/api/health") {
		return
	}
	...
	if method == http.MethodPost {
		if info, err := event.RequestInfo(); err == nil && len(info.Body) > 0 {
			body := make(map[string]any, len(info.Body))
	...
	routine.FireAndForget(func() {
		...
		if err != nil {
			originalLevel = slog.LevelError
		}

		logLevel := originalLevel
		minLevel := slog.Level(event.App.Settings().Logs.MinLevel)
		if logLevel < minLevel {
			logLevel = minLevel
			attrs = append(attrs, slog.String("originalLevel", originalLevel.String()))
		}
		...
		event.App.Logger().Log(context.Background(), logLevel, message, attrs...)
	})
}
