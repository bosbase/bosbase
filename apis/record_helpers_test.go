...
func TestRecordAuthResponseGeneratesStaticTokenForRegularUsers(t *testing.T) {
	app, _ := tests.NewTestApp()
	defer app.Cleanup()
	...
	user.Collection().AuthAlert.Enabled = false

	if err := apis.RecordAuthResponse(event, user, core.MFAMethodPassword, nil); err != nil {
	...
	claims, err := security.ParseUnverifiedJWT(response.Token)
	if err != nil {
		t.Fatalf("Failed to parse auth token: %v", err)
	}

	if cast.ToBool(claims[core.TokenClaimRefreshable]) {
		t.Fatalf("Expected refreshable claim to be false, got %#v", claims[core.TokenClaimRefreshable])
	}

	exp := cast.ToInt64(claims["exp"])
	expiration := time.Unix(exp, 0)
	if time.Until(expiration) < 50*365*24*time.Hour {
		t.Fatalf("Expected auth token expiration to be far in the future, got %v", expiration)
	}
}
