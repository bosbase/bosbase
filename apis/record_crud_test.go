...
func TestRecordCrudList(t *testing.T) {
	...
	scenarios = append(scenarios, tests.ApiScenario{
		Name:   "audit fields on create",
		Method: http.MethodPost,
		URL:    "/api/collections/audit_create/records",
		Body:   strings.NewReader(`{"title":"owned"}`),
		Headers: map[string]string{
			// users, test@example.com
			"Authorization": "eyJhbGciOiJIUzI1NiJ9.eyJpZCI6IjRxMXhsY2xtZmxva3UzMyIsInR5cGUiOiJhdXRoIiwiY29sbGVjdGlvbklkIjoiX3BiX3VzZXJzX2F1dGhfIiwiZXhwIjoyNTI0NjA0NDYxLCJyZWZyZXNoYWJsZSI6dHJ1ZX0.ZT3F0Z3iM-xbGgSG3LEKiEzHrPHr8t8IuHLZGGNuxLo",
		},
		...
		BeforeTestFunc: func(t testing.TB, app *tests.TestApp, e *core.ServeEvent) {
			col := core.NewBaseCollection("audit_create")
			col.Fields.Add(&core.TextField{Name: "title"})
			if err := app.Save(col); err != nil {
				t.Fatalf("failed to create audit collection: %v", err)
			}
		},
		AfterTestFunc: func(t testing.TB, app *tests.TestApp, res *http.Response) {
			...
			recordID, _ := payload["id"].(string)
			if recordID == "" {
				t.Fatalf("missing record id in response payload: %v", payload)
			}

			record, err := app.FindRecordById("audit_create", recordID)
			if err != nil {
				t.Fatalf("failed to load created record: %v", err)
			}
			...
			if got := record.GetString(core.FieldNameCreatedBy); got != user.Id {
				t.Fatalf("expected createdBy %q, got %q", user.Id, got)
			}

			if got := record.GetString(core.FieldNameUpdatedBy); got != user.Id {
				t.Fatalf("expected updatedBy %q, got %q", user.Id, got)
			}
		},
	})
...
func TestRecordCrudCount(t *testing.T) {
	t.Parallel()
	...
	scenarios := []tests.ApiScenario{
		{
			Name:            "missing collection",
			Method:          http.MethodGet,
			URL:             "/api/collections/missing/records/count",
			ExpectedStatus:  404,
			ExpectedContent: []string{`"data":{}`},
			ExpectedEvents:  map[string]int{"*": 0},
		},
		{
			Name:            "unauthenticated trying to access nil rule collection (aka. need superuser auth)",
			Method:          http.MethodGet,
			URL:             "/api/collections/demo1/records/count",
			ExpectedStatus:  403,
			ExpectedContent: []string{`"data":{}`},
			ExpectedEvents:  map[string]int{"*": 0},
		},
		...
		{
			Name:           "public collection - count all records",
			Method:         http.MethodGet,
			URL:            "/api/collections/demo2/records/count",
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`"count":3`,
			},
		...
		{
			Name:           "public collection - count with filter",
			Method:         http.MethodGet,
			URL:            "/api/collections/demo2/records/count?filter=title~'test'",
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`"count":2`,
			},
		...
		{
			Name:           "public collection - count with filter (no matches)",
			Method:         http.MethodGet,
			URL:            "/api/collections/demo2/records/count?filter=title='nonexistent'",
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`"count":0`,
			},
		...
		{
			Name:           "public collection - count with active filter",
			Method:         http.MethodGet,
			URL:            "/api/collections/demo2/records/count?filter=active=true",
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`"count":2`,
			},
		...
		{
			Name:   "superuser - count all records",
			Method: http.MethodGet,
			URL:    "/api/collections/demo1/records/count",
			Headers: map[string]string{
				"Authorization": "eyJhbGciOiJIUzI1NiJ9.eyJpZCI6InN5d2JoZWNuaDQ2cmhtMCIsInR5cGUiOiJhdXRoIiwiY29sbGVjdGlvbklkIjoicGJjXzMxNDI2MzU4MjMiLCJleHAiOjI1MjQ2MDQ0NjEsInJlZnJlc2hhYmxlIjp0cnVlfQ.UXgO3j-0BumcugrFjbd7j0M4MQvbrLggLlcu_YNGjoY",
		...
		{
			Name:           "public collection - count ignores pagination parameters",
			Method:         http.MethodGet,
			URL:            "/api/collections/demo2/records/count?page=2&perPage=1",
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`"count":3`,
			},
	...
	var auditUpdateRecordID string
	auditUpdateScenario := tests.ApiScenario{
		Name:   "audit fields on update",
	...
	auditUpdateScenario.TestAppFactory = func(t testing.TB) *tests.TestApp {
		app, err := tests.NewTestApp()
		if err != nil {
			t.Fatalf("failed to init test app: %v", err)
		}

		col := core.NewBaseCollection("audit_update")
		col.Fields.Add(&core.TextField{Name: "title"})
		if err := app.Save(col); err != nil {
			t.Fatalf("failed to create audit collection: %v", err)
		}
		...
		rec.Set("title", "seed")
		rec.Set(core.FieldNameCreatedBy, user.Id)
		rec.Set(core.FieldNameUpdatedBy, user.Id)
		if err := app.Save(rec); err != nil {
			t.Fatalf("failed to seed record: %v", err)
		}

		auditUpdateRecordID = rec.Id
		auditUpdateScenario.URL = "/api/collections/audit_update/records/" + auditUpdateRecordID

		return app
	}
	auditUpdateScenario.AfterTestFunc = func(t testing.TB, app *tests.TestApp, res *http.Response) {
		record, err := app.FindRecordById("audit_update", auditUpdateRecordID)
		if err != nil {
	...
	scenarios = append(scenarios, auditUpdateScenario)
