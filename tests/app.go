// Package tests provides common helpers and mocks used in BosBase application tests.
package tests
...
func (t *TestApp) Cleanup() {
	...
	for _, cleanup := range t.schemaCleanups {
		cleanup()
	}
	t.schemaCleanups = nil
...
//
// It is the caller's responsibility to call app.Cleanup() when the app is no longer needed.
func NewTestAppWithConfig(config core.BaseAppConfig) (_ *TestApp, err error) {
	if config.DataDir == "" {
	...
	if config.EncryptionEnv != "" {
		if val, ok := os.LookupEnv(config.EncryptionEnv); ok && len(val) > 0 && len(val) < 32 {
			_ = os.Setenv(config.EncryptionEnv, strings.Repeat("a", 32))
		}
	}

	var schemaCleanups []func()
	if config.DBConnect == nil {
		...
		prefix := SanitizeSchemaName(fmt.Sprintf("pbapp_%d", time.Now().UnixNano()))
		if prefix == "" {
			prefix = fmt.Sprintf("pbapp_%d", time.Now().UnixNano())
		}

		var (
			schemaMu    sync.Mutex
			schemaCache = make(map[string]*schemaHandle)
		)

		config.DBConnect = func(dbPath string) (*dbx.DB, error) {
			base := filepath.Base(dbPath)
			schema := SanitizeSchemaName(fmt.Sprintf("%s_%s", prefix, base))
			if schema == "" {
				schema = SanitizeSchemaName(fmt.Sprintf("%s_%d", prefix, time.Now().UnixNano()))
			}

			schemaMu.Lock()
			handle := schemaCache[schema]
			schemaMu.Unlock()
			...
			if handle == nil {
				...
				handle = &schemaHandle{dsn: dsn, cleanup: cleanup}

				schemaMu.Lock()
				schemaCache[schema] = handle
				schemaCleanups = append(schemaCleanups, cleanup)
				schemaMu.Unlock()
			}

			conn, err := dbx.Open("pgx", handle.dsn)
			if err != nil {
				return nil, err
			}
	...
	defer func() {
		if err != nil {
			for _, cleanup := range schemaCleanups {
				cleanup()
			}
		}
	}()
	...
	if len(schemaCleanups) > 0 {
		t.schemaCleanups = append([]func(){}, schemaCleanups...)
	}
	...
	t.OnRecordBindTokenRequest().Bind(&hook.Handler[*core.RecordBindTokenRequestEvent]{
		Func: func(e *core.RecordBindTokenRequestEvent) error {
			t.registerEventCall("OnRecordBindTokenRequest")
			return e.Next()
		},
		Priority: -99999,
	})

	t.OnRecordUnbindTokenRequest().Bind(&hook.Handler[*core.RecordUnbindTokenRequestEvent]{
		Func: func(e *core.RecordUnbindTokenRequestEvent) error {
			t.registerEventCall("OnRecordUnbindTokenRequest")
			return e.Next()
		},
		Priority: -99999,
	})

	t.OnRecordAuthWithTokenRequest().Bind(&hook.Handler[*core.RecordAuthWithTokenRequestEvent]{
		Func: func(e *core.RecordAuthWithTokenRequestEvent) error {
			t.registerEventCall("OnRecordAuthWithTokenRequest")
			return e.Next()
		},
		Priority: -99999,
	})
