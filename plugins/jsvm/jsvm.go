...
// to the BosBase instance (loading migrations, attaching to app hooks, etc.).
//
// Example:
//
...
import (
	...
	"github.com/bosbase/bosbase-enterprise/plugins/jsvm/internal/types/generated"
...
func (p *plugin) registerHooks() error {
	...
	for file, content := range files {
		func() {
			...
			if _, err := loader.RunScript(defaultScriptPath, string(content)); err != nil {
				color.Red("failed to execute %s:\n - %v", file, err)
			}
		}()
	}
