...
// pocketbase.BosBase without the on* hook methods
type BosBase = excludeHooks<ORIGINAL_POCKETBASE>
...
func main() {
	...
	gen := tygoja.New(tygoja.Config{
		Packages: map[string][]string{
			...
			"github.com/bosbase/bosbase-enterprise/tools/template":   {"*"},
	...
	result = strings.ReplaceAll(result, "pocketbase.BosBase", "BosBase")
	result = strings.ReplaceAll(result, "ORIGINAL_CORE_APP", "core.App")
	result = strings.ReplaceAll(result, "ORIGINAL_POCKETBASE", "pocketbase.BosBase")
...
func hooksDeclarations() string {
	...
	for i := 0; i < totalMethods; i++ {
		...
		result.WriteString("/** @group BosBase */")
