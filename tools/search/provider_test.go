...
func createTestDB() (*testDB, error) {
	conn, cleanup, err := openTestSchemaDB(fmt.Sprintf("search_%d", time.Now().UnixNano()))
	if err != nil {
		return nil, err
	}
	...
	if _, err := conn.NewQuery(`DROP TABLE IF EXISTS test`).Execute(); err != nil {
		db.Close()
		return nil, err
	}
...
CREATE TABLE test (
	...
	"%s" TEXT DEFAULT '',
	"%s" TEXT DEFAULT ''
)`, strings.Repeat("a", MaxSortFieldLength), strings.Repeat("b", MaxSortFieldLength+1))
	if _, err := conn.NewQuery(createSQL).Execute(); err != nil {
		db.Close()
		return nil, err
	}

	if _, err := conn.Insert("test", dbx.Params{"id": 1, "test1": 1, "test2": "test2.1"}).Execute(); err != nil {
		db.Close()
		return nil, err
	}
	if _, err := conn.Insert("test", dbx.Params{"id": 2, "test1": 2, "test2": "test2.2"}).Execute(); err != nil {
		db.Close()
		return nil, err
	}
...
// --- minimal Postgres helpers (avoids import cycle with tests package) ---
...
func openTestSchemaDB(schema string) (*dbx.DB, func(), error) {
	...
	conn, err := dbx.Open("pgx", schemaDSN)
	if err != nil {
		cleanup()
		return nil, nil, err
	}
