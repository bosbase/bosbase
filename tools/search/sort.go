...
// This project uses PostgreSQL exclusively, so @rowid maps to the id column.
func (s *SortField) BuildExpr(fieldResolver FieldResolver, driver ...string) (string, error) {
	// special case for random sort
	if s.Name == randomSortKey {
		return "RANDOM()", nil
	}

	// special case for the builtin rowid column (maps to id in PostgreSQL)
