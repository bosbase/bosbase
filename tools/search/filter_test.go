...
func TestFilterDataBuildExpr(t *testing.T) {
	resolver := NewSimpleFieldResolver("test1", "test2", "test3", `^test4_\w+$`, `^test5\.[\w\.\:]*\w+$`)
...
func TestFilterDataBuildExprWithParams(t *testing.T) {
	...
	testDB.QueryLogFunc = func(ctx context.Context, t time.Duration, sql string, rows *sql.Rows, err error) {
		calledQueries = append(calledQueries, sql)
	}
	testDB.ExecLogFunc = func(ctx context.Context, t time.Duration, sql string, result sql.Result, err error) {
		calledQueries = append(calledQueries, sql)
	}
	...
	resolver := NewSimpleFieldResolver(`^test\w+$`)
	...
	testDB.Select().Where(expr).Build().Execute()
...
func TestFilterDataBuildExprWithLimit(t *testing.T) {
	resolver := NewSimpleFieldResolver(`^\w+$`)
