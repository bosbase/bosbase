...
func (s *Provider) Exec(items any) (*Result, error) {
	...
	// Get driver name from FieldResolver if it supports it (e.g., RecordFieldResolver)
	driver := ""
	if resolver, ok := s.fieldResolver.(interface{ DriverName() string }); ok {
	...
	for _, sortField := range s.sort {
		...
		expr, err := sortField.BuildExpr(s.fieldResolver, driver)
		if err != nil {
			return nil, err
		}
		if expr != "" {
