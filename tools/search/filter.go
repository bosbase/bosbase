...
func resolveEqualExpr(equal bool, left, right *ResolverResult) dbx.Expression {
	...
	// a IS b (when both are columns)
	// a != b (when one is a column and the other is a parameter value)
	// a IS NOT b (when both are columns and not equal)
	if left.NoCoalesce || right.NoCoalesce {
		// If one side has parameters (is a value), use != instead of IS NOT
		// IS NOT is only valid for NULL or column-to-column comparisons
		hasLeftParams := len(left.Params) > 0
		hasRightParams := len(right.Params) > 0
		var op string
		if equal {
			if hasLeftParams || hasRightParams {
				// Comparing JSON field to a value
				op = "="
			} else {
				// Comparing two JSON fields (columns)
				op = nullEqualOp
			}
		} else {
			if hasLeftParams || hasRightParams {
				// Comparing JSON field to a value - use != not IS NOT
				op = "!="
			} else {
				// Comparing two JSON fields (columns)
				op = nullEqualOp
		...
		// When comparing a JSON field (jsonb_path_query_first) to a parameter value,
		// we need to cast the parameter to JSONB to match types
		leftIdentifier := left.Identifier
		rightIdentifier := right.Identifier

		// Cast left parameter to JSONB if left is a parameter and right is a JSON field
		if hasLeftParams && right.NoCoalesce {
			// Extract placeholder from left.Identifier and wrap with to_jsonb
			// left.Identifier should be like "{:placeholder}" or something containing it
			if strings.HasPrefix(left.Identifier, "{:") && strings.HasSuffix(left.Identifier, "}") {
				leftIdentifier = fmt.Sprintf("to_jsonb(%s::text)", left.Identifier)
			}
		}

		// Cast right parameter to JSONB if right is a parameter and left is a JSON field
		if hasRightParams && left.NoCoalesce {
			// Extract placeholder from right.Identifier and wrap with to_jsonb
			// right.Identifier should be like "{:placeholder}" or something containing it
			if strings.HasPrefix(right.Identifier, "{:") && strings.HasSuffix(right.Identifier, "}") {
				rightIdentifier = fmt.Sprintf("to_jsonb(%s::text)", right.Identifier)
			}
		}

		return dbx.NewExp(
			fmt.Sprintf("%s %s %s", leftIdentifier, op, rightIdentifier),
