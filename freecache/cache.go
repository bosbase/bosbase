...
// ErrSegmentTimeout indicates that acquiring the segment lock exceeded the provided timeout.
var ErrSegmentTimeout = errors.New("freecache: segment lock timeout")
...
func (cache *Cache) Get(key []byte) (value []byte, err error) {
	value, _, err = cache.getWithExpiration(key, nil, 0, false)
	return value, err
}
...
func (cache *Cache) GetWithExpiration(key []byte) (value []byte, expireAt uint32, err error) {
	return cache.getWithExpiration(key, nil, 0, false)
}

// GetWithTimeout behaves like Get but returns ErrSegmentTimeout if it cannot
// acquire the underlying segment lock within the provided duration.
func (cache *Cache) GetWithTimeout(key []byte, timeout time.Duration) ([]byte, error) {
	value, _, err := cache.getWithExpiration(key, nil, timeout, false)
	return value, err
}

// GetWithExpirationTimeout behaves like GetWithExpiration but aborts with
// ErrSegmentTimeout if the segment lock cannot be acquired before timeout.
func (cache *Cache) GetWithExpirationTimeout(key []byte, timeout time.Duration) ([]byte, uint32, error) {
	return cache.getWithExpiration(key, nil, timeout, false)
}
...
func (cache *Cache) getWithExpiration(key []byte, buf []byte, timeout time.Duration, peek bool) (value []byte, expireAt uint32, err error) {
	hashVal := hashFunc(key)
	segID := hashVal & segmentAndOpVal

	unlock, lErr := cache.lockSegment(segID, timeout)
	if lErr != nil {
		return nil, 0, lErr
	}
	defer unlock()

	return cache.segments[segID].get(key, buf, hashVal, peek)
}

func (cache *Cache) lockSegment(segID uint64, timeout time.Duration) (func(), error) {
