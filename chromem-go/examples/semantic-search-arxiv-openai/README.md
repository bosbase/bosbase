...
2. Set the OpenAI API key in your env as `OPENAI_API_KEY` (the Docker Compose setup forwards this variable into each `bosbase-node`, so you can keep using the same environment variable when running in containers)
