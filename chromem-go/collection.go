...
// ID returns the persistent identifier of the collection.
...
func newCollection(name string, metadata map[string]string, embed EmbeddingFunc, store *pgStore) (*Collection, error) {
	...
	if store != nil {
		c.pgTableName = store.tableNameForCollection(name)
		if c.id == "" {
...
func (c *Collection) ListIDs(ctx context.Context) []string {
	// Always pull the latest state from Postgres (if configured) so every node
	// observes the same set of IDs. Errors fall back to the cached copy.
	_ = c.refreshDocumentsFromStore(ctx)
...
// Metadata returns a copy of the collection metadata.
...
// the collection.
func (c *Collection) ListDocuments(ctx context.Context) ([]*Document, error) {
	if err := c.refreshDocumentsFromStore(ctx); err != nil {
		return nil, err
	}
...
func (c *Collection) ListDocumentsShallow(ctx context.Context) ([]*Document, error) {
	if err := c.refreshDocumentsFromStore(ctx); err != nil {
		return nil, err
	}
...
func (c *Collection) ListDocumentsPartial(ctx context.Context) ([]*Document, error) {
	if err := c.refreshDocumentsFromStore(ctx); err != nil {
		return nil, err
	}
...
func (c *Collection) GetByMetadata(ctx context.Context, where map[string]string) ([]*Document, error) {
	if err := c.refreshDocumentsFromStore(ctx); err != nil {
		return nil, err
	}
...
func (c *Collection) Delete(ctx context.Context, where, whereDocument map[string]string, ids ...string) error {
	...
	for _, docID := range docIDs {
		...
		if err := c.deletePersistedDocument(docID); err != nil {
			return err
		}
	}
...
func (c *Collection) Count() int {
	// Ensure Postgres-backed collections always count the latest persisted docs.
	_ = c.refreshDocumentsFromStore(context.Background())
...
func (c *Collection) queryEmbedding(ctx context.Context, queryEmbedding, negativeEmbeddings []float32, negativeFilterThreshold float32, nResults int, where, whereDocument map[string]string) ([]Result, error) {
	...
	if err := c.refreshDocumentsFromStore(ctx); err != nil {
		return nil, fmt.Errorf("couldn't refresh documents from postgres: %w", err)
	}
...
func (c *Collection) persistMetadata() error {
	...
	return c.pgStore.persistCollectionMetadata(c)
}
...
func (c *Collection) persistDocument(doc *Document) error {
	if c.pgStore == nil {
		return nil
	}
	return c.pgStore.upsertDocument(c, doc)
}

func (c *Collection) deletePersistedDocument(docID string) error {
	if c.pgStore == nil {
		return nil
	}
	return c.pgStore.deleteDocument(c, docID)
}

func (c *Collection) refreshDocumentsFromStore(ctx context.Context) error {
	if c.pgStore == nil {
		return nil
	}
	if ctx == nil {
	...
	if c.pgTableName == "" {
		c.pgTableName = c.pgStore.tableNameForCollection(c.Name)
	}

	docs, err := c.pgStore.loadDocumentsForCollection(ctx, c.pgTableName)
	if err != nil {
		return fmt.Errorf("couldn't load documents from postgres: %w", err)
	}
