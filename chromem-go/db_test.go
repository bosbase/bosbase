...
func TestDB_ImportExportSpecificCollections(t *testing.T) {
	...
	err = newDB.ImportFromFile(filePath, "")
	if err != nil {
		t.Fatal("expected no error, got", err)
	}
	...
	err = newDB.ImportFromFile(filePath2, "")
	if err != nil {
		t.Fatal("expected no error, got", err)
	}

	if len(newDB.collections) != 2 {
		t.Fatalf("expected 2 collections, got %d", len(newDB.collections))
	}
}
