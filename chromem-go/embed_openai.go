...
const defaultBaseURLOpenAI = "https://api.openai.com/v1"
...
// The base URL can be overridden via the environment variable "OPENAI_BASE_URL".
...
func NewEmbeddingFuncOpenAI(apiKey string, model EmbeddingModelOpenAI) EmbeddingFunc {
	...
	normalized := true
	return NewEmbeddingFuncOpenAICompat(resolveOpenAIBaseURL(), apiKey, string(model), &normalized)
}
...
func resolveOpenAIBaseURL() string {
	customBase := strings.TrimSpace(os.Getenv("OPENAI_BASE_URL"))
	if customBase == "" {
