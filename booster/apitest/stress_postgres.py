...
def _worker(i: int, url: str, name_prefix: str, timeout_s: float):
    ...
    try:
        ...
        for m in required_markers:
            if m not in stdout:
                return False, elapsed_ms, f"missing_marker:{m}"

        if name not in stdout:
            return False, elapsed_ms, "missing_name"
...
def main():
    parser = argparse.ArgumentParser(
        description="Stress test booster /run endpoint using the tokio-postgres WASM demo"
    )
    parser.add_argument(
        "--url",
    ...
    parser.add_argument(
        "--name-prefix",
        default="Sparky",
        help="Prefix used to generate unique NAME per request",
    )
    parser.add_argument("--total", type=int, default=10_000, help="Total requests")
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Print progress every N completed requests",
    )
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as ex:
        futures = [
            ex.submit(_worker, i, args.url, args.name_prefix, args.timeout) for i in range(total)
        ]
