    ...
    async function authWithPassword() {
        ...
        try {
            ...
            if (err.status == 401) {
                ...
                // Show the actual error message from API if available, otherwise show generic message
                const message = err.response?.message || err.message || "Invalid login credentials.";
