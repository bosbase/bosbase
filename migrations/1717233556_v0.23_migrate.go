...
func init() {
	core.SystemMigrations.Register(func(txApp core.App) error {
		...
		// Check if the old _params schema exists (with "key" column)
		// If not, this is a fresh install and we should skip the migration
		columns, _ := txApp.TableColumns("_params")
		hasKeyColumn := false
		...
		if !hasKeyColumn {
			// Fresh install or already migrated - skip this migration
			txApp.Logger().Debug("Skipping v0.23 migration: old _params schema not found")
			return nil
		}
...
func migrateOldCollections(txApp core.App, oldSettings *oldSettingsModel) error {
	...
	for _, c := range oldCollections {
		...
		case "auth":
			...
			usernameExpr := "LOWER([[username]])"
			emailCondition := "[[email]] <> ''"

			c.Indexes = append(types.JSONArray[string]{
				fmt.Sprintf("CREATE UNIQUE INDEX _%s_username_idx ON {{%s}} (%s)", c.Id, c.Name, usernameExpr),
				fmt.Sprintf("CREATE UNIQUE INDEX _%s_email_idx ON {{%s}} ([[email]]) WHERE %s", c.Id, c.Name, emailCondition),
