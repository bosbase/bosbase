...
func init() {
	core.SystemMigrations.Register(func(txApp core.App) error {
		// Skip if already initialized (for multi-node race condition safety)
		...
		jsonArrayDefault := core.JSONDefaultLiteral(driver, "[]")
		jsonObjectDefault := core.JSONDefaultLiteral(driver, "{}")

		collectionsSQL := fmt.Sprintf(`
			CREATE TABLE {{_collections}} (
				[[id]]         TEXT PRIMARY KEY DEFAULT %s NOT NULL,
				[[system]]     BOOLEAN DEFAULT FALSE NOT NULL,
				[[type]]       TEXT DEFAULT 'base' NOT NULL,
				[[name]]       TEXT UNIQUE NOT NULL,
				[[fields]]     %s DEFAULT %s NOT NULL,
				[[indexes]]    %s DEFAULT %s NOT NULL,
...
func createParamsTable(txApp core.App) error {
	...
	// Use TEXT instead of JSONB for value column because encrypted values are plain strings,
	// not valid JSON. The JSONRaw type can handle TEXT columns and will work with both
	// JSON strings and encrypted strings.
