...
var (
	supportedLogsMinLevels = []int{
		int(slog.LevelDebug),
		int(slog.LevelInfo),
		int(slog.LevelWarn),
		int(slog.LevelError),
	}
	logsMinLevelValidationValues = []any{
		int(slog.LevelDebug),
		int(slog.LevelInfo),
		int(slog.LevelWarn),
		int(slog.LevelError),
...
const defaultLogsMinLevel = int(slog.LevelInfo)

func isSupportedLogsMinLevel(level int) bool {
	for _, allowed := range supportedLogsMinLevels {
		if level == allowed {
			return true
		}
	}
...
func normalizeLogsMinLevel(level int) int {
	if isSupportedLogsMinLevel(level) {
		return level
	}

	return defaultLogsMinLevel
}
...
func (c LogsConfig) Validate() error {
	return validation.ValidateStruct(&c,
		...
		validation.Field(&c.MinLevel, validation.In(logsMinLevelValidationValues...)),
	)
}
