...
import (
	...
	"github.com/bosbase/bosbase-enterprise/tools/mailer"
...
const (
	...
	DefaultQueryTimeout     time.Duration = 300 * time.Second
...
var ErrS3Required = errors.New("S3 storage must be configured in multi-node mode")
...
// BaseApp implements core.App and defines the base BosBase app structure.
type BaseApp struct {
	...
	onRecordBindTokenRequest            *hook.Hook[*RecordBindTokenRequestEvent]
	onRecordUnbindTokenRequest          *hook.Hook[*RecordUnbindTokenRequestEvent]
	onRecordAuthWithTokenRequest        *hook.Hook[*RecordAuthWithTokenRequestEvent]
...
func NewBaseApp(config BaseAppConfig) *BaseApp {
	...
	if app.config.DataMaxOpenConns <= 0 {
		if envVal := cast.ToInt(os.Getenv("PB_DATA_MAX_OPEN_CONNS")); envVal > 0 {
			app.config.DataMaxOpenConns = envVal
		} else {
			app.config.DataMaxOpenConns = clampInt(effectiveCPUs*30, 8, 512)
		}
	}
	if app.config.DataMaxIdleConns <= 0 {
		if envVal := cast.ToInt(os.Getenv("PB_DATA_MAX_IDLE_CONNS")); envVal > 0 {
			app.config.DataMaxIdleConns = envVal
		} else {
			app.config.DataMaxIdleConns = clampInt(effectiveCPUs*4-1, 2, 64)
		}
	}
	if app.config.AuxMaxOpenConns <= 0 {
		if envVal := cast.ToInt(os.Getenv("PB_AUX_MAX_OPEN_CONNS")); envVal > 0 {
			app.config.AuxMaxOpenConns = envVal
		} else {
			app.config.AuxMaxOpenConns = clampInt(effectiveCPUs*5, 4, 128)
		}
	}
	if app.config.AuxMaxIdleConns <= 0 {
		if envVal := cast.ToInt(os.Getenv("PB_AUX_MAX_IDLE_CONNS")); envVal > 0 {
			app.config.AuxMaxIdleConns = envVal
		} else {
			app.config.AuxMaxIdleConns = clampInt(effectiveCPUs-1, 1, 32)
		}
	}
	if app.config.QueryTimeout <= 0 {
		if envVal := os.Getenv("PB_QUERY_TIMEOUT"); envVal != "" {
			if d, err := time.ParseDuration(envVal); err == nil && d > 0 {
				app.config.QueryTimeout = d
			} else if secs := cast.ToInt64(envVal); secs > 0 {
				app.config.QueryTimeout = time.Duration(secs) * time.Second
			}
		}
		if app.config.QueryTimeout <= 0 {
...
func (app *BaseApp) initHooks() {
	...
	app.onRecordBindTokenRequest = &hook.Hook[*RecordBindTokenRequestEvent]{}
	app.onRecordUnbindTokenRequest = &hook.Hook[*RecordUnbindTokenRequestEvent]{}
	app.onRecordAuthWithTokenRequest = &hook.Hook[*RecordAuthWithTokenRequestEvent]{}
...
func (app *BaseApp) Bootstrap() error {
	...
	err := app.OnBootstrap().Trigger(event, func(e *BootstrapEvent) error {
		...
		if err := app.syncPbHooksFilesystem(); err != nil {
			return err
		}

		if err := app.initLLMStore(); err != nil {
			return err
		}

		if err := app.CacheStore().Warmup(context.Background()); err != nil {
			return err
		}
...
// LLMStore returns the chromem-go store for LLM documents.
func (app *BaseApp) LLMStore() *chromem.DB {
	return app.llmStore
}
...
// RequireS3 reports whether the application forbids falling back to local storage.
func (app *BaseApp) RequireS3() bool {
	if app.config == nil {
		return false
	}

	return app.config.RequireS3
}

// Settings returns the loaded app settings.
func (app *BaseApp) Settings() *Settings {
	app.syncSettingsIfNeeded()
	return app.settings
}

func (app *BaseApp) syncCollectionsIfNeeded() {
	if app == nil || !app.IsBootstrapped() {
		return
	}

	if lastChecked, ok := app.Store().Get(StoreKeyCollectionsLastCheck).(time.Time); ok {
		if time.Since(lastChecked) < collectionsStalenessThreshold {
			return
		}
	}

	app.Store().Set(StoreKeyCollectionsLastCheck, time.Now())

	cachedVersion, _ := app.Store().Get(StoreKeyCollectionsVersion).(types.DateTime)

	var latest types.DateTime
	err := app.DB().NewQuery("SELECT MAX([[updated]]) FROM {{_collections}}").Row(&latest)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		app.Logger().Warn("Failed to check collections version", slog.String("error", err.Error()))
		return
	}

	if latest.IsZero() && cachedVersion.IsZero() {
		return
	}

	if cachedVersion.IsZero() || latest.After(cachedVersion) || (!latest.IsZero() && !latest.Equal(cachedVersion)) {
		if err := app.ReloadCachedCollections(); err != nil {
			app.Logger().Warn("Failed to reload collections cache", slog.String("error", err.Error()))
		}
	}
}

func (app *BaseApp) syncSettingsIfNeeded() {
	if app == nil || !app.IsBootstrapped() {
		return
	}

	if lastChecked, ok := app.Store().Get(StoreKeySettingsLastCheck).(time.Time); ok {
		if time.Since(lastChecked) < settingsStalenessThreshold {
			return
		}
	}

	// optimistic timestamp to avoid concurrent requests hammering the db
	app.Store().Set(StoreKeySettingsLastCheck, time.Now())

	cachedVersion, _ := app.Store().Get(StoreKeySettingsVersion).(types.DateTime)

	param := &Param{}
	err := app.ModelQuery(param).
		Select("{{" + paramsTable + "}}.*").
		AndWhere(dbx.HashExp{"id": paramsKeySettings}).
		Limit(1).
		One(param)
	if err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			app.Logger().Warn("Failed to check settings version", slog.String("error", err.Error()))
		}
		return
	}

	if param.Updated.IsZero() {
		return
	}

	if cachedVersion.IsZero() || !param.Updated.Equal(cachedVersion) {
		if err := app.ReloadSettings(); err != nil {
			app.Logger().Warn("Failed to reload settings", slog.String("error", err.Error()))
		}
	}
}
...
// CacheStore returns the cache store used for SDK cache operations.
func (app *BaseApp) CacheStore() *CacheStore {
	if app.cacheStore == nil {
		app.cacheStore = NewCacheStore(app)
	}
...
func (app *BaseApp) OnRecordBindTokenRequest(tags ...string) *hook.TaggedHook[*RecordBindTokenRequestEvent] {
	return hook.NewTaggedHook(app.onRecordBindTokenRequest, tags...)
}

func (app *BaseApp) OnRecordUnbindTokenRequest(tags ...string) *hook.TaggedHook[*RecordUnbindTokenRequestEvent] {
	return hook.NewTaggedHook(app.onRecordUnbindTokenRequest, tags...)
}

func (app *BaseApp) OnRecordAuthWithTokenRequest(tags ...string) *hook.TaggedHook[*RecordAuthWithTokenRequestEvent] {
	return hook.NewTaggedHook(app.onRecordAuthWithTokenRequest, tags...)
}
...
func (app *BaseApp) initAuxDB() error {
	...
	// (see https://github.com/bosbase/bosbase-enterprise/issues/5607)
...
func (app *BaseApp) initLLMStore() error {
	builder, ok := app.nonconcurrentDB.(interface {
		DB() *sql.DB
	})
	if !ok || builder == nil {
		return errors.New("nonconcurrent DB handle is not available")
	}

	llmDB, err := chromem.NewPostgresDB(context.Background(), builder.DB())
	if err != nil {
		return err
	}
...
func (app *BaseApp) registerBaseHooks() {
	...
	app.OnModelAfterDeleteSuccess().Bind(&hook.Handler[*ModelEvent]{
		...
		Func: func(e *ModelEvent) error {
			if m, ok := e.Model.(FilesManager); ok && m.BaseFilesPath() != "" && supportFiles(e.Model) {
				...
				// (https://github.com/bosbase/bosbase-enterprise/discussions/5246#discussioncomment-10128955)
	...
	app.registerTokenBindingsHooks()
	app.registerActivationHooks()
}
...
func (app *BaseApp) initLogger() error {
	...
	handler := logger.NewBatchHandler(logger.BatchOptions{
		...
		WriteFunc: func(ctx context.Context, logs []*logger.Log) error {
			...
			// check ENABLE_DB_LOG environment variable
			// default to true (enabled) if not set
			enableDBLog := true
			if envVal := os.Getenv("ENABLE_DB_LOG"); envVal != "" {
				enableDBLog = cast.ToBool(envVal)
			}

			// skip database writes if ENABLE_DB_LOG is set to false
			if !enableDBLog {
				return nil
			}
			...
			app.AuxRunInTransaction(func(txApp App) error {
				...
				for _, l := range logs {
					model.MarkAsNew()
					model.Id = generateSequentialLogId()
