...
const (
	// authTokenCacheTTL is the TTL for token->record cache entries.
	// Using 30 minutes as a balance between performance and token validity checks.
	authTokenCacheTTL = 30 * time.Minute
)

var (
	authTokenCacheOnce sync.Once
	authTokenCache     *freecache.Cache
)

func getAuthTokenCache() *freecache.Cache {
	authTokenCacheOnce.Do(func() {
		// 4MB cache for auth token lookups
		authTokenCache = freecache.NewCache(4 * 1024 * 1024)
	})
	return authTokenCache
}

// InvalidateAuthTokenCache clears all cached auth token lookups.
func InvalidateAuthTokenCache() {
	getAuthTokenCache().Clear()
}

// InvalidateAuthTokenCacheEntry removes a specific token from the cache.
func InvalidateAuthTokenCacheEntry(token string) {
	getAuthTokenCache().Del([]byte(token))
}
...
func (app *BaseApp) FindRecordsByFilter(
	...
	if sort != "" {
		// Get driver name (RecordFieldResolver has DriverName() method)
		driver := resolver.DriverName()
		for _, sortField := range search.ParseSortFromString(sort) {
			expr, err := sortField.BuildExpr(resolver, driver)
			if err != nil {
				return nil, err
			}
			if expr != "" {
...
// authTokenCacheEntry stores cached record data for token lookups.
...
func (app *BaseApp) FindAuthRecordByToken(token string, validTypes ...string) (*Record, error) {
	...
	cache := getAuthTokenCache()
	cacheKey := []byte(token)

	if cached, err := cache.Get(cacheKey); err == nil {
		var entry authTokenCacheEntry
		if json.Unmarshal(cached, &entry) == nil && entry.CollectionID == collectionId && entry.RecordID == id {
			// reconstruct record from cache
			collection, err := app.FindCollectionByNameOrId(entry.CollectionID)
			if err == nil && collection.IsAuth() {
				record := NewRecord(collection)
				record.Load(entry.Data)
				// still need to verify token signature
				...
				if _, err := security.ParseJWT(token, secret); err == nil {
					return record, nil
				}
				// token signature invalid, remove from cache
				cache.Del(cacheKey)
			}
		}
	}
	...
	if data, err := json.Marshal(entry); err == nil {
		cache.Set(cacheKey, data, int(authTokenCacheTTL.Seconds()))
	}
...
func (app *BaseApp) FindAuthRecordByEmail(collectionModelOrIdentifier any, email string) (*Record, error) {
	...
	if ok && (strings.EqualFold(index.Columns[0].Collate, "nocase") || strings.HasPrefix(strings.ToUpper(strings.TrimSpace(index.Columns[0].Name)), "LOWER(")) {
		// case-insensitive search
		driver := BuilderDriverName(app.NonconcurrentDB())
		expr = dbx.NewExp(CaseInsensitiveEqExpr(FieldNameEmail, "{:email}", driver), dbx.Params{"email": email})
	} else {
