...
func (f *TextField) ColumnType(app App) string {
	...
	if f.PrimaryKey {
		...
		return fmt.Sprintf("TEXT PRIMARY KEY DEFAULT %s NOT NULL", RandomIDExpr(driver))
	}
	...
	if f.PrimaryKey {
		...
		if !record.IsNew() {
			...
			if oldVal != "" {
				...
				// that may not comply with the user defined BosBase validations
				return nil
			}
		} else {
			...
			if f.Pattern != defaultRecordIdPattern {
				var exists int
				driver := BuilderDriverName(app.NonconcurrentDB())
				expr := CaseInsensitiveEqExpr("id", "{:id}", driver)
				err := app.ConcurrentDB().
					Select("(1)").
					From(record.TableName()).
					Where(dbx.NewExp(expr, dbx.Params{"id": newVal})).
					Limit(1).
					Row(&exists)
...
func (f *TextField) Intercept(
	...
	case InterceptorActionValidate, InterceptorActionCreate:
		if f.Name == FieldNameId && f.PrimaryKey && record.IsNew() && f.hasZeroValue(record) {
			record.SetRaw(f.Name, GenerateDefaultRandomId())
		}
