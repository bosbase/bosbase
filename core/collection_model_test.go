...
func TestCollectionIdFieldDefaultsAfterLoad(t *testing.T) {
	t.Parallel()
	...
	idField, _ := collection.Fields.GetByName(core.FieldNameId).(*core.TextField)
	if idField == nil {
		t.Fatal("id field not found")
	}

	if idField.Min != core.DefaultIdLength {
		t.Fatalf("expected id min %d, got %d", core.DefaultIdLength, idField.Min)
	}

	if idField.Max != core.DefaultIdLength {
		t.Fatalf("expected id max %d, got %d", core.DefaultIdLength, idField.Max)
	}

	expectedPattern := "^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"
	if idField.Pattern != expectedPattern {
		t.Fatalf("expected id pattern %q, got %q", expectedPattern, idField.Pattern)
	}

	createdField, _ := collection.Fields.GetByName(core.FieldNameCreated).(*core.AutodateField)
	if createdField == nil {
		t.Fatal("created field not found")
	}
	if !createdField.OnCreate || createdField.OnUpdate {
		t.Fatalf("expected created field OnCreate=true and OnUpdate=false, got %v/%v", createdField.OnCreate, createdField.OnUpdate)
	}

	updatedField, _ := collection.Fields.GetByName(core.FieldNameUpdated).(*core.AutodateField)
	if updatedField == nil {
		t.Fatal("updated field not found")
	}
	if !updatedField.OnCreate || !updatedField.OnUpdate {
		t.Fatalf("expected updated field OnCreate=true and OnUpdate=true, got %v/%v", updatedField.OnCreate, updatedField.OnUpdate)
	}

	createdByField, _ := collection.Fields.GetByName(core.FieldNameCreatedBy).(*core.TextField)
	if createdByField == nil {
		t.Fatal("createdBy field not found")
	}
	if !createdByField.System || createdByField.Hidden {
		t.Fatalf("expected createdBy field to be system and visible, got %v/%v", createdByField.System, createdByField.Hidden)
	}

	updatedByField, _ := collection.Fields.GetByName(core.FieldNameUpdatedBy).(*core.TextField)
	if updatedByField == nil {
		t.Fatal("updatedBy field not found")
	}
	if !updatedByField.System || updatedByField.Hidden {
		t.Fatalf("expected updatedBy field to be system and visible, got %v/%v", updatedByField.System, updatedByField.Hidden)
	}
}
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			...
			rowErr := app.ConcurrentDB().NewQuery(`
				SELECT pg_get_viewdef(oid, true) AS sql 
				FROM pg_catalog.pg_views 
				WHERE schemaname = current_schema() AND viewname = {:name}
			`).Bind(dbx.Params{"name": viewName}).Row(&sql)
			if rowErr != nil {
