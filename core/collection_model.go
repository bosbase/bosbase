...
const (
	defaultRecordIdPattern            = "^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"
	legacyRecordIdPattern             = "^[a-z0-9]+$"
	legacyRecordIdAutogeneratePattern = `[a-z0-9]{15}`
)

const (
	defaultOpenAccessRule    = ""
	defaultAuthenticatedOnly = "@request.auth.id != ''"
	defaultRecordOwnerRule   = "@request.auth.id != '' && createdBy = @request.auth.id"
)
...
type Collection struct {
	...
	// SkipDefaultFields prevents automatic default field injection (except the id field).
	SkipDefaultFields bool `db:"-" json:"-" form:"-"`
	// SkipTableSync prevents automatic schema synchronization with the database table.
	SkipTableSync bool `db:"-" json:"-" form:"-"`
}
...
func (m *Collection) applyDefaultApiRulesIfEmpty() {
	if m == nil {
		return
	}

	if (m.IsBase() || m.IsView()) && m.ListRule == nil {
		m.ListRule = types.Pointer(defaultOpenAccessRule)
	}

	if (m.IsBase() || m.IsView()) && m.ViewRule == nil {
		m.ViewRule = types.Pointer(defaultOpenAccessRule)
	}
	...
	if m.CreateRule == nil {
		m.CreateRule = types.Pointer(defaultAuthenticatedOnly)
	}

	if m.UpdateRule == nil {
		m.UpdateRule = types.Pointer(defaultRecordOwnerRule)
	}

	if m.DeleteRule == nil {
		m.DeleteRule = types.Pointer(defaultRecordOwnerRule)
	}
}
...
func (m *Collection) PostScan() error {
	...
	if err := m.unmarshalRawOptions(); err != nil {
		return err
	}

	m.SkipDefaultFields = m.collectionBaseOptions.ExternalTable
	m.SkipTableSync = m.collectionBaseOptions.ExternalTable
...
func onCollectionSave(e *CollectionEvent) error {
	...
	e.Collection.SkipDefaultFields = e.Collection.collectionBaseOptions.ExternalTable
	e.Collection.SkipTableSync = e.Collection.collectionBaseOptions.ExternalTable
...
func (c *Collection) initDefaultFields() {
	...
	case CollectionTypeBase:
		c.initIdField()
		if c.SkipDefaultFields {
			return
		}
		c.initCreatedField()
		c.initUpdatedField()
		c.initCreatedByField()
		c.initUpdatedByField()
	case CollectionTypeAuth:
		c.initIdField()
		if c.SkipDefaultFields {
			return
...
func (c *Collection) initIdField() {
	...
	if field == nil {
		...
		field = &TextField{
			...
			Min:        DefaultIdLength,
			Max:        DefaultIdLength,
			Pattern:    defaultRecordIdPattern,
		}
		...
		field.Hidden = false
		if !c.collectionBaseOptions.ExternalTable {
			field.Min = DefaultIdLength
			field.Max = DefaultIdLength
			if field.Pattern == "" || field.Pattern == legacyRecordIdPattern {
				field.Pattern = defaultRecordIdPattern
			}
			if field.AutogeneratePattern == legacyRecordIdAutogeneratePattern {
...
func (c *Collection) initCreatedField() {
	field, _ := c.Fields.GetByName(FieldNameCreated).(*AutodateField)
	if field == nil {
...
func (c *Collection) initUpdatedField() {
	field, _ := c.Fields.GetByName(FieldNameUpdated).(*AutodateField)
	if field == nil {
		c.Fields.Add(&AutodateField{
...
func (c *Collection) initCreatedByField() {
	field, _ := c.Fields.GetByName(FieldNameCreatedBy).(*TextField)
	if field == nil {
		if !c.IsNew() {
			return
		}
...
func (c *Collection) initUpdatedByField() {
	field, _ := c.Fields.GetByName(FieldNameUpdatedBy).(*TextField)
	if field == nil {
		if !c.IsNew() {
			return
		}
