...
type App interface {
	...
	// RequireS3 reports whether local filesystem fallbacks are disabled
	// and S3 must be configured.
	RequireS3() bool
	...
	// LLMStore returns the chromem-go instance that powers the LLM document APIs.
	LLMStore() *chromem.DB
	...
	BindCustomToken(authRecord *Record, token string) error

	// UnbindCustomToken removes a token binding from the provided auth record.
	UnbindCustomToken(authRecord *Record, token string) error

	// FindAuthRecordByCustomToken returns the auth record associated with the provided custom token.
	FindAuthRecordByCustomToken(collectionModelOrIdentifier any, token string) (*Record, error)

	// DeleteAllTokenBindingsByRecord deletes all token bindings associated with the provided record.
	DeleteAllTokenBindingsByRecord(authRecord *Record) error

	// DeleteAllTokenBindingsByCollection deletes all token bindings associated with the provided collection.
	DeleteAllTokenBindingsByCollection(collection *Collection) error
	...
	// OnRecordBindTokenRequest hook is triggered on each Record bind custom token API request.
	//
	// Could be used to validate the request data or implement custom persistence.
	//
	// If the optional "tags" list (Collection ids or names) is specified,
	// then all event handlers registered via the created hook will be
	// triggered and called only if their event data origin matches the tags.
	OnRecordBindTokenRequest(tags ...string) *hook.TaggedHook[*RecordBindTokenRequestEvent]

	// OnRecordUnbindTokenRequest hook is triggered on each Record unbind custom token API request.
	//
	// Could be used to validate the request data or implement custom persistence.
	//
	// If the optional "tags" list (Collection ids or names) is specified,
	// then all event handlers registered via the created hook will be
	// triggered and called only if their event data origin matches the tags.
	OnRecordUnbindTokenRequest(tags ...string) *hook.TaggedHook[*RecordUnbindTokenRequestEvent]

	// OnRecordAuthWithTokenRequest hook is triggered on each Record auth with custom token API request.
	//
	// Could be used to validate the request data or implement custom authentication behavior.
	//
	// If the optional "tags" list (Collection ids or names) is specified,
	// then all event handlers registered via the created hook will be
	// triggered and called only if their event data origin matches the tags.
	OnRecordAuthWithTokenRequest(tags ...string) *hook.TaggedHook[*RecordAuthWithTokenRequestEvent]
