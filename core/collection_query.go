...
const (
	StoreKeyCachedCollections     = "pbAppCachedCollections"
	StoreKeyCollectionsVersion    = "pbAppCollectionsVersion"
	StoreKeyCollectionsLastCheck  = "pbAppCollectionsLastCheck"
	collectionsStalenessThreshold = 500 * time.Millisecond
)
...
func (app *BaseApp) FindAllCollections(collectionTypes ...string) ([]*Collection, error) {
	...
	// Use id for ordering (PostgreSQL)
...
func (app *BaseApp) ReloadCachedCollections() error {
	...
	for _, c := range collections {
		if c.Updated.After(latest) {
			latest = c.Updated
		}
	}
	app.Store().Set(StoreKeyCollectionsVersion, latest)
...
func (app *BaseApp) FindCachedCollectionByNameOrId(nameOrId string) (*Collection, error) {
	...
	if reloadErr := app.ReloadCachedCollections(); reloadErr != nil {
		app.Logger().Warn("Failed to reload collections cache", "error", reloadErr)
	}
...
// (see https://github.com/bosbase/bosbase-enterprise/issues/3110).
