...
func (r *runner) processRequestInfoEachModifier(bodyField Field) (*search.ResolverResult, error) {
	...
	if r.withMultiMatch {
		...
		jeTable2 := fmt.Sprintf("(SELECT jsonb_array_elements_text({:%s}::jsonb) AS value)", placeholder2)
		...
		r.multiMatch.params[placeholder2] = bodyItemsJSON
	...
	for i, prop := range r.activeProps {
		...
		if field == nil {
			...
			if backRelField.CollectionId != collection.Id {
				// https://github.com/bosbase/bosbase-enterprise/discussions/6590#discussioncomment-12496581
				if r.nullifyMisingField {
					return &search.ResolverResult{Identifier: "NULL"}, nil
				}
...
func (r *runner) processLastProp(collection *Collection, prop string) (*search.ResolverResult, error) {
	...
	// (https://github.com/bosbase/bosbase-enterprise/issues/4068)
