...
func TestBaseAppNewFilesystem(t *testing.T) {
	...
	// RequireS3 no longer enforces S3 requirement - local filesystem is allowed
	clusterApp := core.NewBaseApp(core.BaseAppConfig{
		DataDir:   testDataDir,
		RequireS3: true,
	})
	defer clusterApp.ResetBootstrapState()

	clusterFS, clusterErr := clusterApp.NewFilesystem()
	if clusterErr != nil {
		t.Fatalf("Expected no error when RequireS3 is true, got %v", clusterErr)
	}
	if clusterFS == nil {
		t.Fatal("Expected local filesystem instance when RequireS3 is true, got nil")
	}
}
