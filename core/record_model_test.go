...
func TestRecordSaveIdFromOtherCollection(t *testing.T) {
	...
	r2.Set("id", core.GenerateDefaultRandomId())
	if err := app.Save(r2); err != nil {
		t.Fatalf("Expected nil, got error %v", err)
	}
}
...
func TestRecordSaveIdUpdateNoValidation(t *testing.T) {
	...
	rec.Id = core.GenerateDefaultRandomId()
...
func TestRecordDelete(t *testing.T) {
	...
	expectedRelManyPart := "SELECT `demo1`.* FROM `demo1` WHERE EXISTS (SELECT 1 FROM (SELECT jsonb_array_elements_text(CASE WHEN [[demo1.rel_many]] IS NULL OR [[demo1.rel_many]]::text = 'null' THEN '[]'::jsonb WHEN jsonb_typeof([[demo1.rel_many]]::jsonb) = 'array' THEN [[demo1.rel_many]]::jsonb ELSE jsonb_build_array([[demo1.rel_many]]::jsonb) END) AS value) {{__je__}} WHERE [[__je__.value]]='"
