...
func TestSettingsAutoReloadOnExternalUpdate(t *testing.T) {
	t.Parallel()
	...
	initialVersion, _ := app.Store().Get(core.StoreKeySettingsVersion).(types.DateTime)
	if initialVersion.IsZero() {
		t.Fatal("expected settings version to be initialized")
	}
	...
	defer secondary.ResetBootstrapState()

	if err := secondary.Bootstrap(); err != nil {
		t.Fatalf("secondary bootstrap failed: %v", err)
	}

	secondary.Settings().Meta.AppName = "multi_node_update"
	if err := secondary.Save(secondary.Settings()); err != nil {
		t.Fatalf("secondary settings save failed: %v", err)
	}

	app.Store().Set(core.StoreKeySettingsLastCheck, time.Time{})

	if got := app.Settings().Meta.AppName; got != "multi_node_update" {
		t.Fatalf("expected settings to reload with new app name, got %q", got)
	}
}
