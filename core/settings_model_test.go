...
func TestLogsConfigValidate(t *testing.T) {
	scenarios := []struct {
		...
		{
			"invalid min level",
			core.LogsConfig{MinLevel: 3},
			[]string{"minLevel"},
		},
		{
			"valid min level debug",
			core.LogsConfig{MinLevel: int(slog.LevelDebug)},
			[]string{},
		},
		{
			"valid data",
...
func TestSettingsPostScanNormalizesLogsMinLevel(t *testing.T) {
	t.Parallel()

	settings := core.Settings{}
	settings.Logs.MinLevel = 3

	if err := settings.PostScan(); err != nil {
		t.Fatalf("Expected nil PostScan error, got %v", err)
	}

	if settings.Logs.MinLevel != int(slog.LevelInfo) {
		t.Fatalf("Expected logs min level to fallback to %d, got %d", int(slog.LevelInfo), settings.Logs.MinLevel)
	}

	settingsValid := core.Settings{}
	settingsValid.Logs.MinLevel = int(slog.LevelWarn)

	if err := settingsValid.PostScan(); err != nil {
		t.Fatalf("Expected nil PostScan error, got %v", err)
	}

	if settingsValid.Logs.MinLevel != int(slog.LevelWarn) {
		t.Fatalf("Expected logs min level to remain %d, got %d", int(slog.LevelWarn), settingsValid.Logs.MinLevel)
	}
}
