...
const (
	StoreKeySettingsVersion    = "pbAppSettingsVersion"
	StoreKeySettingsLastCheck  = "pbAppSettingsVersionLastCheck"
	settingsStalenessThreshold = 500 * time.Millisecond
)
...
func (app *BaseApp) ReloadSettings() error {
	...
	if param.Id == "" {
		...
		app.settingsReloadMu.Lock()
		app.settings.MarkAsNew()
		app.settingsReloadMu.Unlock()
		return app.Save(app.settings)
	}

	app.settingsReloadMu.Lock()
	defer app.settingsReloadMu.Unlock()

	event := new(SettingsReloadEvent)
	event.App = app

	return app.OnSettingsReload().Trigger(event, func(e *SettingsReloadEvent) error {
		if base, ok := e.App.(*BaseApp); ok {
			if err := base.settings.loadParam(base, param); err != nil {
				return err
			}

			base.Store().Set(StoreKeySettingsVersion, param.Updated)
			base.Store().Set(StoreKeySettingsLastCheck, time.Now())

			return nil
		}

		if err := e.App.Settings().loadParam(e.App, param); err != nil {
			return err
		}

		e.App.Store().Set(StoreKeySettingsVersion, param.Updated)
		e.App.Store().Set(StoreKeySettingsLastCheck, time.Now())
